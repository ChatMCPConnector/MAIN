...
- `infra/scripts/cline-models.py`: Cline-Free-Modelle live geprüft, Alias
  ...
  `opencode.json` ab. Ohne Key geht `--no-probe` (nur Liste). Cache 1 h unter
  `~/.cache/cline-models-cache.json`; die Modellliste selbst wird immer frisch
  geholt, weil Free-Modelle rotieren.
