...
Step war bisher eine volle Seite. Der Filter rastet deshalb pro Richtung ein
**Fensterfenster** ein: `FREEBUFF_WHEEL_DEBOUNCE_MS`, Default **120 ms**, d. h.
 hoechstens ~8 Seiten/s, und die verworfenen Bytes werden **konsumiert** (nicht
nur verworfen) — sonst rueutscht der rohe Pfeil an der App vorbei. Genau diesen
Bug hat der Testfall „High-Resolution-Rad“ gefunden; er stand im ersten Wurf des
Reglers drin. Werte: `0` = keine Drosselung (frueheres Verhalten), `250` = sehr
zurueckhaltend. Ein bewusst langsamer Notch-Abstand (> 120 ms) ist von der
Drosselung nicht betroffen, laesst sich also weiterhin Seite fuer Seite
blattern. Live am pty verifiziert: vier Notches in 200 ms -> **eine** `ESC[5~`.
