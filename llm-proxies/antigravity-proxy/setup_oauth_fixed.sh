...
# Step 2: Stop any existing proxy (WITHOUT killing this script!)
...
# Step 4: Run OAuth flow using the auth helper
info "Starting OAuth flow..."
cd /workspaces/dvcrn-antigravity-oauth-proxy

# The auth command will:
# 1. Print OAuth URL
# 2. Open browser automatically (unless --no-browser)
# 3. Wait for callback OR accept manual paste
# 4. Exchange code for tokens
# 5. Save credentials to oauth_creds.json

# Run without --no-browser flag so it opens the browser
./auth 2>&1 | tee /tmp/oauth-setup.log || {
    warn "OAuth flow with auto-browser failed, trying manual mode..."
    ./auth --no-browser 2>&1 | tee /tmp/oauth-setup.log || {
        error "OAuth flow failed completely"
