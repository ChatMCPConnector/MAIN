...
label() {
  case "$1" in
    ...
    8000) echo "Python/FastAPI";; 8001) echo "glm2api-Proxy";;
...
if command -v gh >/dev/null 2>&1; then
  ...
  if [ -n "${cs:-}" ]; then
    gh codespace ports --codespace "$cs" --json label,sourcePort,visibility,browseUrl 2>/dev/null | python3 -c "
import json, sys, socket
try:
    ports = json.load(sys.stdin)
    for p in ports:
        port = p.get('sourcePort', 0)
        lbl = p.get('label') or ''
        url = p.get('browseUrl') or ''
        s = socket.socket()
        s.settimeout(0.1)
        alive = (s.connect_ex(('127.0.0.1', port)) == 0)
        s.close()
        tag = '[AKTIV]  ' if alive else '[INAKTIV]'
        lbl_str = f' ({lbl})' if lbl else ''
        print(f'  {tag}  {port:<5}{lbl_str:<22} {url}')
except Exception as e:
    print('  (Fehler beim Parsen der Ports)')
